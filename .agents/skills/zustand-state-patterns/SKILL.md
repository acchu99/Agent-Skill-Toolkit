...
## Description
Best practices for managing application state in MyApp using Zustand and the slice pattern.
...
## MyApp Workspace State Rules

- Keep feature stores scoped to the active space when the underlying data is space-scoped.
- When fetching a new space's list, clear `selected*Id` and selected detail if the current selection is not present in the returned list.
- Do not allow a URL query param to select an item from another space. Validate the ID against the current-space list before selecting it.
